...
标志是一个插入符，旁边三行文字依次变短。它由 `lib/logo.ts` 里的一组坐标画出来，
标题栏、首页和关于页读的都是这一份。

## 代码签名

安装包**没有**签名，而且不存在既免费又全自动的签名方式。Windows SmartScreen 依据的是
签发给已核验身份的证书，加上文件本身的下载信誉；它完全忽略自签名证书——自己签一个不会
有任何改变。

| 方案 | 费用 | 需要做什么 |
| ---- | ---- | ---------- |
| 不签名直接发布 | 免费 | 用户会看到一次“Windows 已保护你的电脑”，可以继续运行 |
| [SignPath.io](https://signpath.io) 开源项目免费 | 免费 | 提交申请并通过审核，之后在 CI 里自动签名 |
| Azure Trusted Signing | 约 $10/月 | 先做身份核验，之后全自动 |
| OV 或 EV 证书 | $200–600/年 | 身份核验是最慢的一步 |

构建已经为这几种方式做好了准备：在环境变量里设置 `CSC_LINK`（`.pfx` 或指向它的链接）和
`CSC_KEY_PASSWORD`，electron-builder 就会自动签名，仓库里不需要改任何东西。
